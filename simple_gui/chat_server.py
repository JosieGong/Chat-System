...
#==============================================================================
                ...
                for g in the_guys[1:]:
                    to_sock = self.logged_name2sock[g]
                    #self.indices[g].add_msg_and_index(said2)
                    #print({"action":"game", "from":msg["from"]})                    
                    mysend(to_sock, json.dumps({"action":"game", "from":msg["from"]}))
#==============================================================================
#                 rank
#==============================================================================            
                ...
                the_guys = self.group.list_me(from_name)
                ##said = msg["from"]+msg["message"]
                if from_name in self.game_record:
                    if self.game_record[from_name]<=msg["score"]:
                        self.game_record[from_name] = msg["score"] 
                    else:
                        pass                    
                else:
                    self.game_record[from_name]= msg["score"]   
                ...
                from_name = self.logged_sock2name[from_sock]
                sorted_game_record = dict(sorted(self.game_record.items(), key=lambda x: x[1], reverse=True))
                results=''
                for index, (key, value) in enumerate(sorted_game_record.items(), start=1):
                    results+= f"{index}  {key}  {value}\n"
                mysend(from_sock, json.dumps({"action":"rank", "results":results}))
